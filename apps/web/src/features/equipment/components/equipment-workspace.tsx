...
function AssignmentForm({ equipment, onClose }: Readonly<{ equipment: Equipment; onClose: () => void }>) {
  ...
  return (
    <div className="finance-modal-backdrop" role="presentation"><section className="finance-modal" role="dialog" aria-modal="true" aria-labelledby="equipment-assign-title"><header className="finance-modal-header"><div><p className="eyebrow">{equipment.code} · {equipment.name}</p><h2 id="equipment-assign-title">Assign to Project / Stage</h2></div><button type="button" className="finance-modal-close" onClick={onClose} aria-label="Close assignment">×</button></header><div className="finance-modal-body"><form className="admin-grid two-columns" onSubmit={form.handleSubmit((values) => void handleSubmit(values))}>
        ...
        <p className="muted">Estimated amount: {estimate ?? 'Enter valid assignment details'} · {toDate ? 'selected date range' : `one ${equipment.rateUnit?.toLowerCase() ?? 'rate'} unit until completion`}</p>
...
function EquipmentLedgerModal({ equipment, onClose }: Readonly<{ equipment: Equipment; onClose: () => void }>) {
  const history = useEquipmentHistory(equipment.id, true);
  const postedTotal = history.data?.usage.reduce((total, row) => total + Number(row.amount), 0) ?? 0;

  return <div className="finance-modal-backdrop" role="presentation"><section className="finance-modal finance-modal-wide" role="dialog" aria-modal="true" aria-labelledby="equipment-ledger-title"><header className="finance-modal-header"><div><p className="eyebrow">{equipment.code} · {equipment.name}</p><h2 id="equipment-ledger-title">Equipment Ledger</h2><p>{equipment.equipmentType} · {equipment.ownershipType === 'RENTED' ? 'Rented' : 'Owned'} · {equipment.defaultRate ?? '0.0000'} / {(equipment.rateUnit ?? 'unit').toLowerCase()}</p></div><button type="button" className="finance-modal-close" onClick={onClose} aria-label="Close equipment ledger">×</button></header><div className="finance-modal-body">{history.isPending && <p className="finance-modal-state">Loading equipment ledger…</p>}{errorMessage(history.error) && <div className="form-error">{errorMessage(history.error)}</div>}{history.data && <div className="admin-stack"><div className="equipment-ledger-summary"><span><small>Assignment status</small><strong>{equipment.assignmentStatus === 'ASSIGNED' ? 'Assigned' : 'Available'}</strong></span><span><small>Posted project cost</small><strong>{postedTotal.toFixed(2)}</strong></span><span><small>Cost entries</small><strong>{history.data.usage.length}</strong></span></div><h3>Project assignments</h3><div className="table-scroll"><table><thead><tr><th>Project / Stage</th><th>Period</th><th>Quantity</th><th>Rate</th><th>Estimated</th><th>Status</th></tr></thead><tbody>{history.data.assignments.map((row) => <tr key={row.id}><td><strong>{row.projectName ?? 'Project'}</strong><br /><small>{row.stageName ?? 'Project level'}</small></td><td>{row.fromDate} {row.fromTime} → {row.toDate ? `${row.toDate} ${row.toTime ?? ''}` : 'In progress'}</td><td>{row.quantity}</td><td>{row.rate} / {row.rateUnit.toLowerCase()}</td><td>{row.estimatedAmount ?? '—'}</td><td>{row.status === 'ACTIVE' ? 'Assigned' : 'Completed'}</td></tr>)}{history.data.assignments.length === 0 && <tr><td colSpan={6} className="muted">No project assignments yet.</td></tr>}</tbody></table></div><h3>Cost ledger</h3><div className="table-scroll"><table><thead><tr><th>Posting date</th><th>Project / Stage</th><th>Usage</th><th>Rate</th><th>Posted cost</th><th>Status</th></tr></thead><tbody>{history.data.usage.map((row) => <tr key={row.id}><td>{row.usageDate}</td><td><strong>{row.projectName ?? 'Project'}</strong><br /><small>{row.stageName ?? 'Project level'}</small></td><td>{row.quantity} hours/units</td><td>{row.rate}</td><td>{row.amount}</td><td>{row.status}</td></tr>)}{history.data.usage.length === 0 && <tr><td colSpan={6} className="muted">No project cost has been posted for this equipment.</td></tr>}</tbody></table></div><h3>Maintenance history</h3><div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Cost</th><th>Note</th><th>Status</th></tr></thead><tbody>{history.data.maintenance.map((row) => <tr key={row.id}><td>{row.maintenanceDate}</td><td>{row.type}</td><td>{row.cost}</td><td>{row.note ?? '—'}</td><td>{row.status}</td></tr>)}{history.data.maintenance.length === 0 && <tr><td colSpan={5} className="muted">No maintenance entries.</td></tr>}</tbody></table></div></div>}</div></section></div>;
}
...
export function EquipmentWorkspace(props: EquipmentWorkspaceProps) {
  ...
  return (
    <div className="admin-stack">
      <section className="equipment-workflow-bar"><div><strong>Equipment workflow</strong><span>Add equipment → Assign to project/stage → Complete assignment → Cost posted automatically</span></div>{props.canManage && <button type="button" onClick={() => setEditor('create')}>Add Equipment</button>}</section>
