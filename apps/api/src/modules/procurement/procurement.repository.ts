...
export class ProcurementRepository {
  ...
  async listPurchaseOrders(input: PageWindow & Readonly<{ visibility: ProjectVisibility; projectId?: string }>) {
    ...
    const where = scope.where({ ...projectVisibilityWhere(input.visibility), ...(input.projectId ? { projectId: input.projectId } : {}) });
    const [items, total] = await Promise.all([
      this.db.purchaseOrder.findMany({ where, include: { items: { include: { inventoryItem: { select: { name: true } } }, orderBy: [{ id: 'asc' }] }, goodsReceipts: { include: { items: { select: { acceptedQty: true, sourceUnitCost: true } } }, orderBy: [{ receivedAt: 'desc' }, { id: 'desc' }] } }, orderBy: [{ orderDate: 'desc' }, { id: 'desc' }], skip: input.skip, take: input.take }),
  ...
  async findPurchaseOrderById(purchaseOrderId: string, visibility: ProjectVisibility) {
    const scope = requireCompanyRepositoryScope();
    return this.db.purchaseOrder.findFirst({ where: scope.where({ id: purchaseOrderId, ...projectVisibilityWhere(visibility) }), include: { items: { include: { inventoryItem: { select: { name: true } } }, orderBy: [{ id: 'asc' }] }, goodsReceipts: { include: { items: { select: { acceptedQty: true, sourceUnitCost: true } } }, orderBy: [{ receivedAt: 'desc' }, { id: 'desc' }] } } });
  }
  ...
  async createPurchaseOrder(input: Readonly<{ projectId: string; requisitionId: string; poNo: string; vendorId: string; orderDate: Date; currency: string; subtotal: string; taxAmount: string; totalAmount: string; deliveryAddress: string; terms: string; items: readonly PurchaseOrderLineWrite[] }>) {
    ...
    return this.db.purchaseOrder.create({
      ...
      include: { items: { include: { inventoryItem: { select: { name: true } } }, orderBy: [{ id: 'asc' }] }, goodsReceipts: { include: { items: { select: { acceptedQty: true, sourceUnitCost: true } } }, orderBy: [{ receivedAt: 'desc' }, { id: 'desc' }] } }
    });
  }
  ...
  async updatePurchaseOrderStatus(purchaseOrderId: string, expectedStatus: string, status: string, cancellation?: Readonly<{ reason: string; actorUserId: string; at: Date }>) {
    ...
    if (result.count === 0) return null;
    return this.db.purchaseOrder.findFirst({ where: scope.where({ id: purchaseOrderId }), include: { items: { include: { inventoryItem: { select: { name: true } } }, orderBy: [{ id: 'asc' }] }, goodsReceipts: { include: { items: { select: { acceptedQty: true, sourceUnitCost: true } } }, orderBy: [{ receivedAt: 'desc' }, { id: 'desc' }] } } });
  }
