...
export class ProjectsService {
  ...
  async updateProject(projectId: string, input: UpdateProjectBody) {
    ...
    return withTransaction(this.db, async (tx) => {
      ...
      if (before.projectModel === PROJECT_MODEL_COST_PLUS_PERCENTAGE
        && nextProjectModel === PROJECT_MODEL_FIXED_PRICE
        && await repository.hasStageCostPlusPercent(projectId)) {
        throw new ValidationError({
          fieldErrors: [{
            field: 'projectModel',
            message: 'Clear Stage Profit / Markup overrides before switching this Project to Fixed Price.'
          }]
        });
      }
