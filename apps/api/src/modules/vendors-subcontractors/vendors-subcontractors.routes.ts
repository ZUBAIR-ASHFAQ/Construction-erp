...
const SUBCONTRACT_PAYMENT_LIST_QUERY_JSON_SCHEMA = { type: 'object', additionalProperties: false, properties: { subcontractorId: UUID_JSON_SCHEMA, projectId: UUID_JSON_SCHEMA, subcontractContractId: UUID_JSON_SCHEMA, fromDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' }, toDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' }, status: { type: 'string', enum: ['DRAFT', 'POSTED'] }, ...PAGE_PROPERTIES } } as const;
