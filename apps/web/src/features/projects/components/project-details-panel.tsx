      ...
      <div className="project-summary-section">
        ...
        {details.expenseSummary ? <>
          <dl className="project-expense-grid">
            {details.expenseSummary.categories.map((row) => <div key={row.category}><dt>{({ material: 'Material', labour: 'Employee Salaries', security: 'Security Employee Salaries', equipment: 'Equipment Expense', subcontract: 'Subcontractor', site_expense: 'Site expenses', other: 'Other expenses' } as const)[row.category]}</dt><dd>{project.currency} {row.amount}</dd></div>)}
            ...
            <div className="project-expense-total"><dt>Total actual project cost</dt><dd>{project.currency} {details.expenseSummary.totalExpense}</dd><small>Includes finalized Employee Salaries and the agreed subcontract contract amount without counting later cash settlements twice.</small></div>
          </dl>
