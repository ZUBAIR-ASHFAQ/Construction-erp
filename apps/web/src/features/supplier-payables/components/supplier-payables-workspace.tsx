            ...
            <div className="finance-modal-body">
              ...
              <form className="admin-form client-payment-create-form" onSubmit={paymentForm.handleSubmit(submitPayment)}>
                <div className="client-payment-create-grid">
                  ...
                  <label>Choose payment proof (optional)<input key={paymentProofInputKey} type="file" accept="image/jpeg,image/png,application/pdf" disabled={!props.canUploadDocuments || !props.canLinkDocuments} onChange={(event) => setPaymentProof(event.target.files?.[0] ?? null)} /><small className="muted">Choose a bank slip, receipt image, or PDF. After posting, use Edit proof to replace it through version history.</small></label>
                </div>
